...
class Synapse(nn.Module):
    ...
    def forward(self, input_spike, t):
        ...
        if self.pre_type == "e":
            self.ge = mask*(self.ge+torch.abs(self.weight.weight))+(1.0-mask)*self.ge*(1.0-1.0/1)
        else:
            self.gi = mask*(self.gi+torch.abs(self.weight.weight))+(1.0-mask)*self.gi*(1.0-1.0/2)
...
def test(args):
    ...
    # 需要指定目前训练得到的checkpoint的epoch和iteration
    load_epoch = 0
    load_iter = 0
